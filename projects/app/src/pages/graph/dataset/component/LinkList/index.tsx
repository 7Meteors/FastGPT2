...
const LinkList = () => {
  ...
  const { links, Pagination, total, getData, isGetting, pageNum, pageSize } = useContextSelector(
    LinkListPageContext,
    (v) => v
  );
  ...
  const formatCollections = useMemo(
    () =>
      links.map((link) => {
        const icon = getCollectionIcon(link.type, link.name);
        const status = (() => {
          if (link.trainingAmount > 0) {
            return {
              statusText: t('dataset.collections.Collection Embedding', {
                total: link.trainingAmount
              }),
              colorSchema: 'gray'
            };
          }
  ...
  const isLoading =
    isUpdating || isDeleting || isSyncing || (isGetting && links.length === 0) || isDropping;
                    ...
                    if (collection.type === DatasetCollectionTypeEnum.folder) {
                      ...
                      router.push({
                        query: {
                          ...
                          currentTab: DatasetTypeEnum.dataCard
                        }
                      });
                    }
                  }}
